...
# obj_1 = GameObjects(obj.id, (obj.x, obj.y), obj.image)
        ...
        for i in range(len(world)):
            for j in range(len(world[i])):
                if pos == (world[i][j].rect.x, world[i][j].rect.y):
                    # props = tiled_map.get_tile_properties(i, j, 2)
                    if True:
                        try:
...
def mapper(layer):
    ...
    for x, y, image in layer[0].tiles():
        m[x][y] = Tile((x * tile_size, y * tile_size), layer[1], image)
    return m


def draw_map(tile_map):
    for x, y, image in tile_map[0].tiles():
